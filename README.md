...
Второй флоу — упрощённый режим по прямой ссылке: http://localhost:3000/invitation?n=Ирина открывает INVITATION сразу, без дерева имён и вопроса.
...
## Прямые ссылки для гостей (упрощённый режим)

Для гостей, которым проще открыть готовую ссылку, есть режим без дерева имён и без вопроса:

```
https://sacred-castle-wedding.ru/invitation?n=Ирина
https://sacred-castle-wedding.ru/invitation?n=Никита&g=m   # род задан явно
```

- `n` — имя гостя, показывается в приветствии («Дорогая Ирина»). Кириллицу можно писать как есть — браузер закодирует сам.
- `g` — необязательный род для приветствия: `f` («Дорогая») или `m` («Дорогой»). Без него род определяется автоматически: имя, совпавшее с гостем из `src/data/guests.ts`, берёт род оттуда, остальные — по окончанию имени (со списком мужских исключений вроде «Никита», «Саша» в `src/lib/directGuest.ts`). Если приветствие вышло неправильным — добавить `&g=`.
- `/invitation` без `n` перенаправляет на обычный ENTRY.

Ссылка не защищена вопросом-верификацией — это осознанное упрощение для конкретных гостей, а не дыра в проверке (сама проверка и так не является security-границей, см. `CLAUDE.md` → Verification & Privacy).
...
Поля `question` и `answers` — опциональные, но задаются только вместе (иначе скрипт упадёт с ошибкой):

- гость **с** вопросом (whitelist) — при тапе по яблоку открывается модалка с личным вопросом и проверкой ответа;
- гость **без** вопроса — открывается та же модалка, но с общим приветствием «Добро пожаловать в наш замок любви» и сразу с кнопкой «Открыть приглашение».
...
- Иллюстрации карты усадьбы и конверта — текстовые заглушки (`PlaceholderIllustration`), арт ещё не создан.
